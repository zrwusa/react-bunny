...
class MedianFinder {
    ...
    addNum(num: number): void {
        if (this._leftHeap.size === 0) {
            this._leftHeap.offer(num);
        } else {
            const leftPeek = this._leftHeap.peek();
            if (leftPeek !== null) {
                if (num > leftPeek) {
                    this._rightHeap.offer(num);
                } else {
                    this._leftHeap.offer(num);
                }
            }
        ...
        if (leftSize - rightSize >= 2) {
            ...
            if (leftPolled) {
                this._rightHeap.offer(leftPolled);
            }
        } else if (rightSize > leftSize) {
            // TODO after no-non-null-assertion not ensure the logic
            const rightPolled = this._rightHeap.poll();
            if (rightPolled) {
                this._leftHeap.offer(rightPolled);
            }
        }
    }
...
export function kthSmallestInSortedMatrix(matrix: number[][], k: number): number {
    ...
    minHeap.offer({val: matrix[0][0], y: 0, x: 0});
    ...
    while (--k > 0) {
        ...
        if (y < matrix.length - 1 && matrix[y + 1][x] !== Infinity) {
            minHeap.offer({val: matrix[y + 1][x], y: y + 1, x});
            matrix[y + 1][x] = Infinity;
        }

        if (x < matrix[0].length - 1 && matrix[y][x + 1] !== Infinity) {
            minHeap.offer({val: matrix[y][x + 1], y, x: x + 1});
