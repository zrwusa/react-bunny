...
import {BinaryIndexedTree, BST} from 'data-structure-typed';
...
const countSmallerNoReverse = function (nums: number[]): number[] {
    ...
    const freqs = new Array(uniqueSorted.length).fill(0);
...
export const countSmallerBST = async (nums: number[], proxyHandler: TProxyHandler) => {
    ...
    proxyVariables.bst.put(nums[rootIndex], nums[rootIndex])
    const outputArr = new Array(nums.length).fill(0);

    for (let j = nums.length - 1; j > -1; j--) {
        if (j !== rootIndex) {
            const node = proxyVariables.bst.put(nums[j], nums[j]);
            // outputArr[j] = node?.allLesserSum; // todo
        }
    }
    return outputArr;
};
...
function smallestDistancePair(nums: number[], k: number): number {
    ...
    while (min < max - 1) {
        ...
        const count = countLessThan(nums, mid);
